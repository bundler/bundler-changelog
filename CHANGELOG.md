/* This file is automatically generated from https://github.com/bundler/bundler-changelog */
...
Features:
  ...
  - require passing `--all` to `bundle update` to update all gems ([#2646](https://github.com/bundler/issues/2646), @segiddins, @jhhere)
  - bundler will prefer a `gems.rb` file in the same directory as a `Gemfile` (@segiddins)
  - allow conflicts between bundler dependencies and the current bundler version ([#3871](https://github.com/bundler/issues/3871), @segiddins)
  - running `bundle` without an explicit command prints a concise help message instead of installing ([#3831](https://github.com/bundler/issues/3831), [#5963](https://github.com/bundler/issues/5963), @segiddins)
  - add a global cache for downloaded `.gem` files ([#3983](https://github.com/bundler/issues/3983), @segiddins, @smlance)
  - pin gems to only come either from the source they were declared to come from (the top-level source if declared at the top-level of the Gemfile, else a scoped source), or a source that it transitively "inherited" from the gems that required it ([#3671](https://github.com/bundler/issues/3671), [#3696](https://github.com/bundler/issues/3696), [#4059](https://github.com/bundler/issues/4059), @segiddins)
  - suppress `using...` messages during installation when the version of a gem has not changed ([#3872](https://github.com/bundler/issues/3872), @segiddins)
  - the `bundle cache` command will behave the same as the `package` command ([#4008](https://github.com/bundler/issues/4008), @segiddins)
  ...
  - only print the version number when running `bundle --version` ([#4706](https://github.com/bundler/issues/4706), @koic)
  - globally cache extensions after they have been built, dramatically speeding up installation of the same gem into different projects ([#5905](https://github.com/bundler/issues/5905), @segiddins)
  ...
  - `bundle install` will automatically clean `./.bundle` of unused gems when no explicit path has been set ([#5875](https://github.com/bundler/issues/5875), @segiddins)
  - add subcommands to `bundle config` (`list`, `get`, `set`, and `unset`) ([#4600](https://github.com/bundler/issues/4600), [#5507](https://github.com/bundler/issues/5507), @denniss, @segiddins)
...
Bugfixes:
  ...
  - ensure that the git cache is per-user when using per-app installation paths ([#5895](https://github.com/bundler/issues/5895), @segiddins)
  - installation will use `jobs` jobs, rather than `jobs -1` ([#5808](https://github.com/bundler/issues/5808), @tjschuck, @segiddins)
  - an explicit relative `path` is expanded relative to the current working directory when set ([#2048](https://github.com/bundler/issues/2048), @igorbozato, @segiddins)
...
Deprecations:
  ...
  - remove the `github.https` setting ([#5698](https://github.com/bundler/issues/5698), @segiddins)
  - remove the `github` gemfile git source shortcut ([#5699](https://github.com/bundler/issues/5699), @segiddins)
  ...
  - remove the `bundle console` command in favor of `bundle gem`'s `bin/console` binstub ([#4025](https://github.com/bundler/issues/4025), [#4034](https://github.com/bundler/issues/4034), @segiddins, @Zorbash)
  - remove the `frozen` setting in favor of `deployment` ([#4619](https://github.com/bundler/issues/4619), @segiddins)
  - the bundler CLI will no longer persist options and flags ([#3955](https://github.com/bundler/issues/3955), @segidins)
  - the `bundle viz` command has been extracted into a plugin ([#5180](https://github.com/bundler/issues/5180), @colby-swandale)
  - `bundle outdated` has `--strict` renamed to `--filter-strict` and `--update-strict` to `--strict` ([#5081](https://github.com/bundler/issues/5081), @segiddins)
...
Bugfixes:
  ...
  - fix a bug where installing on FreeBSD would accidentally raise an error ([#6013](https://github.com/bundler/issues/6013), @olleolleolle)
  ...
  - the outdated bundler version message is disabled by default until the message has been fine-tuned ([#6004](https://github.com/bundler/issues/6004), @segiddins)
...
Features:
  ...
  - allow using non-branch symbolic refs in a git source ([#4845](https://github.com/bundler/issues/4845), @segiddins)
  - allow absolute paths in the `cache path` setting ([#5627](https://github.com/bundler/issues/5627), @mal)
  - gems created via `bundle gem` with rspec have `--require spec_helper` in their `.rspec` file (@koic)
  - `bundle env` includes `Gem.ruby` and the `bundle` binstub shebang when they don't match ([#5616](https://github.com/bundler/issues/5616), @segiddins)
  - allow passing gem names to `bundle pristine` (@segiddins)
  - `bundle version` and `bundle env` include the commit and build date for the bundler gem ([#5049](https://github.com/bundler/issues/5049), @segiddins)
  - add the `--shebang` option to `bundle binstubs` ([#4070](https://github.com/bundler/issues/4070), @segiddins, @Zorbash)
  - gemfiles are `eval`ed one fewer time when running `bundle install` ([#4952](https://github.com/bundler/issues/4952), [#3096](https://github.com/bundler/issues/3096), [#4417](https://github.com/bundler/issues/4417), @segiddins)
  - the `fileutils` gem is now vendored so different versions of the gem can be activated (@segiddins)
  - speed up no-op installations ([#5842](https://github.com/bundler/issues/5842), @segiddins)
  - default to keeping the lockfile in the default gem template (@deivid-rodriguez)
  - add a special bundler binstub that ensures the correct version of bundler is activated ([#5876](https://github.com/bundler/issues/5876), @segiddins)
  - speed up dependency resolution and ensure that all resolvable gemfiles can be installed (@segiddins, @greysteil)
  - add a `bundle list` command that prints the gems in use ([#4754](https://github.com/bundler/issues/4754), @colby-swandale)
...
Bugfixes:
  ...
  - allow configuring a mirror fallback timeout without a trailing slash ([#4830](https://github.com/bundler/issues/4830), @segiddins)
  ...
  - ensure `Bundler.original_env` preserves all env keys that bundler sets ([#5700](https://github.com/bundler/issues/5700), @segiddins)
  - ensure `bundle pristine` removes files added to a git gem (@segiddins)
  - load plugin files from path gems before gem installation ([#5429](https://github.com/bundler/issues/5429), @segiddins)
  - ensure gems containing manpages are properly set up ([#5730](https://github.com/bundler/issues/5730), @segiddins)
  - avoid fetching remote specs when all effected gems are in groups that are not being installed (@segiddins)
  - allow `BUNDLE_GEMFILE` to be a relative path ([#5712](https://github.com/bundler/issues/5712), @gxespino)
  - show a more helpful error message when a gem fails to install due to a corrupted lockfile ([#5846](https://github.com/bundler/issues/5846), @segiddins)
  - add a process lock to allow multiple concurrent `bundle install`s ([#5851](https://github.com/bundler/issues/5851), @stefansedich)
  ...
  - avoid global namespace pollution when loading gems ([#5958](https://github.com/bundler/issues/5958), @shyouhei)
  ...
  - fix support for bundle paths that include jars or wars on jruby ([#5975](https://github.com/bundler/issues/5975), @torcido)
