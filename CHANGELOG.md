...
Features:
  ...
  - require passing `--all` to `bundle update` to update all gems ([#2646](https://github.com/bundler/bundler/issues/2646), @segiddins, @jhhere)
  - bundler will prefer a `gems.rb` file in the same directory as a `Gemfile` (@segiddins)
  - allow conflicts between bundler dependencies and the current bundler version ([#3871](https://github.com/bundler/bundler/issues/3871), @segiddins)
  - running `bundle` without an explicit command prints a concise help message instead of installing ([#3831](https://github.com/bundler/bundler/issues/3831), [#5963](https://github.com/bundler/bundler/issues/5963), @segiddins)
  - add a global cache for downloaded `.gem` files ([#3983](https://github.com/bundler/bundler/issues/3983), @segiddins, @smlance)
  - pin gems to only come either from the source they were declared to come from (the top-level source if declared at the top-level of the Gemfile, else a scoped source), or a source that it transitively "inherited" from the gems that required it ([#3671](https://github.com/bundler/bundler/issues/3671), [#3696](https://github.com/bundler/bundler/issues/3696), [#4059](https://github.com/bundler/bundler/issues/4059), @segiddins)
  - suppress `using...` messages during installation when the version of a gem has not changed ([#3872](https://github.com/bundler/bundler/issues/3872), @segiddins)
  - the `bundle cache` command will behave the same as the `package` command ([#4008](https://github.com/bundler/bundler/issues/4008), @segiddins)
  ...
  - only print the version number when running `bundle --version` ([#4706](https://github.com/bundler/bundler/issues/4706), @koic)
  - globally cache extensions after they have been built, dramatically speeding up installation of the same gem into different projects ([#5905](https://github.com/bundler/bundler/issues/5905), @segiddins)
  ...
  - `bundle install` will automatically clean `./.bundle` of unused gems when no explicit path has been set ([#5875](https://github.com/bundler/bundler/issues/5875), @segiddins)
  - add subcommands to `bundle config` (`list`, `get`, `set`, and `unset`) ([#4600](https://github.com/bundler/bundler/issues/4600), [#5507](https://github.com/bundler/bundler/issues/5507), @denniss, @segiddins)
  - bundler automatically uses `nproc` jobs when installing gems ([#5808](https://github.com/bundler/bundler/issues/5808), @segiddins)
  - the resolver will error when it is ambiguous which source a gem should come from ([#4629](https://github.com/bundler/bundler/issues/4629), @segiddins)
  - allow users to override `~/.bundle` locations by setting the `BUNDLE_USER_{HOME,CACHE,CONFIG,PLUGIN}` environment variables ([#4333](https://github.com/bundler/bundler/issues/4333), @gwerbin)
  - add a `bundle plugin list` command ([#5672](https://github.com/bundler/bundler/issues/5672), [#5467](https://github.com/bundler/bundler/issues/5467), @jumbosushi)

Bugfixes:

  - `Kernel#gem` is no longer made public (@segiddins)
  - ensure that the git cache is per-user when using per-app installation paths ([#5895](https://github.com/bundler/bundler/issues/5895), @segiddins)
  - installation will use `jobs` jobs, rather than `jobs -1` ([#5808](https://github.com/bundler/bundler/issues/5808), @tjschuck, @segiddins)
  - an explicit relative `path` is expanded relative to the current working directory when set ([#2048](https://github.com/bundler/bundler/issues/2048), @igorbozato, @segiddins)
  - fail gracefully when creating a temporary directory for the compact index fails ([#5518](https://github.com/bundler/bundler/issues/5518), @ajwann)
  - invalid `.gem` files will be removed instead of being cached ([#5941](https://github.com/bundler/bundler/issues/5941), @segiddins)
  - avoid using a shell in the Rakefile gem helper tasks (@nobu)
  - support `bundle outdated --group` with the second group for a dependency ([#6115](https://github.com/bundler/bundler/issues/6115), @segiddins)
  - execute git in the gem's directory by default in the gempsec generated by `bundle gem` (@elia)

Deprecations:

  - remove the `github.https` setting ([#5698](https://github.com/bundler/bundler/issues/5698), @segiddins)
  - remove the `github` gemfile git source shortcut ([#5699](https://github.com/bundler/bundler/issues/5699), @segiddins)
  ...
  - remove the `bundle console` command in favor of `bundle gem`'s `bin/console` binstub ([#4025](https://github.com/bundler/bundler/issues/4025), [#4034](https://github.com/bundler/bundler/issues/4034), @segiddins, @Zorbash)
  - remove the `frozen` setting in favor of `deployment` ([#4619](https://github.com/bundler/bundler/issues/4619), @segiddins)
  - the bundler CLI will no longer persist options and flags ([#3955](https://github.com/bundler/bundler/issues/3955), @segidins)
